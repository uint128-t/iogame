...
class circularArray {
    constructor(size,sprite){
        this.buffer = new Array(size).fill(sprite);
    ...
    add(value) {
    // Try to find the next inactive slot starting at head
    let idx = this.head;
    let found = false;

    for (let i = 0; i < this.size; i++) {
      if (!this.buffer[idx].visible) {
        found = true;
        break;
      }
      idx = (idx + 1) % this.size;
    ...
    if (!found) {
      // All slots active → overwrite at head
      idx = this.head;
      this.head = (this.head + 1) % this.size;
    }

    this.buffer[idx].visible=true;
    this.buffer[idx].x = value.x;
    this.buffer[idx].y = value.y;
    this.buffer[idx].vx = value.vx;
    this.buffer[idx].vy = value.vy;
    this.buffer[idx].age = 0;
  }

    
    
    getAll(){
        if (!this.full) return this.buffer.slice(0,this.head);
...
app.init({
    ...
    var drawnBullets = new circularArray(10000,PIXI.Sprite(circleTexture));

    function drawBullet(x,y,dx,dy){
        drawnBullets.add(x,y,dx,dy);
    }
    

    app.ticker.add((delta) => {
        for (bullet of drawnBullets.getAll()){
            if (bullet.visible){
                bullet.x += bullet.vx * delta.DeltaTime;
                bullet.y += bullet.vy * delta.DeltaTime;
                bullet.age += delta.DeltaTime;
                if (bullet.age > 1000){
